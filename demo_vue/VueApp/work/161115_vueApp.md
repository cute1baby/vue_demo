...
* 使用vue-cli快速创建工程化项目
  * 使用基于webpack的简单模板创建项目: webpack-simple_demo
    ...
    ```
    ```
    注意:如果全局安装vue-cli不行的话，可以通过淘宝镜像的方法尝试，例如下面：
...
* 组件间通信
 ...
      this.$dispatch(eventName, data): 分给父辈组件(冒泡)【可以跨越多层父子级之间进行通信】
...
* 基本使用编码
 ...
  //通过vue组件对象发送ajax请求【类似es6中promise的写法】
...
* 相关API说明
  ...
  * 指令与组件:
    * v-link: 用来指定路由路径
      ```
      <a v-link='{path:"/about"}'>About</a>      相当于之前使用的href属性，但是不能直接跳转。
     ```
    * <router-view>: 用来显示当前路由组件界面
      ```
      <router-view></router-view>    里面存放主体资料
      ```
* 实现简单路由
  * 路由组件:
    * home.vue
  ...
  * 应用组件: App.vue
    ...
    <div>
      ...
      <!--用于渲染当前路由组件-->
      <router-view keep-alive></router-view>    keep-alive:这个属性在路由组件发生变化的时候同时保存当前的值
    </div>
  ...
  * 入口js: main.js
    ...
    //初始请求一个路由
    router.go('/about');        //在vue1.0需要指定初始访问额路由组件。
...
* 路由请求携带参数
  * 配置路由
    ...
    subRoutes: {
      '/mdetail/:id': {
        component: messageDetail   这种情况是在一个页面里面还有tab项进行切换时候导致的。
     }
    }
  ...
  * 路由路径
    ```
    <a v-link="{path: '/home/message/mdetail/2'}">{{m.title}}</a>      把这里的2换成变量id
   
    ```
  * 路由组件中读取请求参数
    ```
