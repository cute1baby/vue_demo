...
todo_list得到的启发：
1、【试过了，确实可以】今天突然想到一个问题，就是我们在做vue项目的时候，有时候会遇到子组件修改变量传数据给父组件的时候，使用$emit调用父组件的自定义函数。
但是现在想要一个方法是，父组件往子组件传一个方法并且调用这个方法也能达到效果。

2、如果在父级定义了一个变量直接传该变量给子级组件，那么在子级直接获取是会报错的。
解决方案1：
 但是如果数据要在父级定义的话，那么就只能【传对象】过去，修改对象里面的属性时不会导致报错的。
解决方案2：
 直接定义【变量在子组件】中。
```

slider_scroll文件夹是个人封装的一个自定义组件。
-------------------------------------------------------------------------------------------------
```
...
```
# 正确配置stylus的方法：
(1)安装stylus,stylus-loader两个依赖
（2）在组件中的style中添加一些这样的内容：
