<template>
    <div id="right">
      <div class="right_top">
        <div class="data_left">
          <span>审计周期:&nbsp;&nbsp;</span>
          <span class="dateNow">{{timeNow}}</span>
        </div>
        <div class="switch_right">
          <span>快速切换:&nbsp;&nbsp;</span>
          <button type="button" class="btn btn-custom" datanum="1" @click="handleTime(1)" :class="{dateShow:timeIndex==1}">今天</button>
          <button type="button" class="btn btn-custom" datanum="2" @click="handleTime(2)" :class="{dateShow:timeIndex==2}">最近7天</button>
          <button type="button" class="btn btn-custom" datanum="3" @click="handleTime(3)" :class="{dateShow:timeIndex==3}">当前月份</button>
          <button type="button" class="btn btn-custom" datanum="4" @click="handleTime(4)" :class="{dateShow:timeIndex==4}">近三个月</button>
        </div>
      </div>

        <ul class="contentTable" v-if="perGroup==0">
          <!-- (1)个人聊天对象 -->
          <li v-if="nowIndex==0">
            <div class="chatTable">
          ...
          <li v-if="nowIndex==3">
            <div class="chatTable2">
              ...
              <div class="pageNacation rankPersonTerms">
                <table>
                  <tbody>
                  <tr colspan="3">
                    <td>无符合条件的记录</td>
        ...
        <ul class="contentTable" v-if="perGroup==1">
          <!-- (1)群组聊天内容 -->
          <li v-if="nowIndex==0">
            <div class="chatTable2">
...
<script>
    export default {
      name: "subRight",
      props:['nowIndex','perGroup','timeIndex','timeNow'],
      methods:{
        handleTime(index){
          alert(index)
          this.$emit('time-index-fun',index)
        }
      }
    }
</script>
